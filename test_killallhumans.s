...
nextchar1:
        ...
        bra     nextchar1
.text:
        byte    "Kill all humans?",CR,LF,0

; read stream of chars into a buffer, echoing and handling back delete
read:
        ldx     #0
        stz     buffer,x        ; initialise buffer
.next_char:
        jsr     serial_get_char
        cmp     #DEL
        beq     .delete
...
respond:                        ;jsr lcd_clear
        ldx     #0
.loop:
        lda     buffer,x        ;  reading chars sent
        beq     .done
        ; check is lower case
        cmp     #"a"
        bcc     .send           ; <
